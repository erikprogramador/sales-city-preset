        ...
        <div class="flex justify-center px-5 sm:px-0 pb-16">
            <div class="w-full sm:w-1/3 bg-white shadow rounded overflow-hidden">
                <header class="text-center p-6">
                    ...
                    <h2 class="font-semibold text-sales-darkest mt-1 text-lg">Seja bem vindo!</h2>
                </header>

                <form action="{{ route('register') }}" method="POST" class="mb-6 px-6">
                    @csrf

                    <div class="form mb-4{{ $errors->has('name') ? ' has-errors' : '' }}">
                        <label for="name">Nome</label>

                        <input type="text" name="name" id="name" placeholder="jhon@example.com" value="{{ old('name') }}" autofocus required>
